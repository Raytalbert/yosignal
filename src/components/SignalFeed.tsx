...
export function SignalFeed({
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <Sheet open={!!active} onOpenChange={(v) => { if (!v) { setActive(null); setSignalTab("talk"); setAgentResult(null); setAgentCmd(""); } }}>
        <SheetContent side="right" className="w-full sm:max-w-lg flex flex-col p-3 sm:p-6">
          {active && (
            <div className="flex flex-col h-full min-h-0">
              {/* Tab switcher */}
              <div className="flex gap-1 mb-4 bg-card border border-border/60 rounded-full p-1 shrink-0">
                <button
                  type="button"
                  onClick={() => setSignalTab("talk")}
                  className={`flex-1 text-xs font-mono uppercase tracking-wider py-1.5 rounded-full transition ${signalTab === "talk" ? "bg-signal text-signal-foreground" : "text-muted-foreground hover:text-foreground"}`}
                >
                  Talk
                </button>
                <button
                  type="button"
                  onClick={() => setSignalTab("agent")}
                  className={`flex-1 text-xs font-mono uppercase tracking-wider py-1.5 rounded-full transition ${signalTab === "agent" ? "bg-signal text-signal-foreground" : "text-muted-foreground hover:text-foreground"}`}
                >
                  Yo, Signal ⚡
                </button>
              </div>
              {/* Talk tab */}
              {signalTab === "talk" && <SignalThread startup={startup} signal={active} />}
              {/* Agent tab */}
              {signalTab === "agent" && (
                <div className="flex flex-col h-full min-h-0 gap-3">
                  <div className="space-y-1">
                    <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Signal loaded</p>
                  ...
                  {agentResult ? (
                    <div className="flex-1 overflow-y-auto space-y-3">
                      <div className="bg-card border border-signal/30 rounded-lg p-4">
                        <p className="font-mono text-[9px] uppercase tracking-wider text-signal mb-3">{agentCmd}</p>
                        <div className="text-sm leading-relaxed text-foreground/90 whitespace-pre-wrap">{agentResult}</div>
                      </div>
                      <button
                        type="button"
                        onClick={() => { setAgentResult(null); setAgentCmd(""); }}
                        className="w-full text-xs font-mono uppercase tracking-wider text-muted-foreground border border-border/60 rounded-md py-2 hover:border-signal/60 hover:text-signal transition"
                      >
                        Run another command
                      </button>
                      <button
                    ...
                    <>
                      <div className="flex flex-col gap-2 shrink-0">
                        <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Quick commands</p>
                        {["Turn this into a marketing plan", "Give me 3 action items for this week", "Write an opportunity brief", "Analyze the competitive angle", "What should I do about this today?"].map(cmd => (
                          <button
                            key={cmd}
                            type="button"
                            ...
                            onClick={() => {
                              ...
                              runAgent({
                                ...
                                if (window.speechSynthesis) {
                                  ...
                                  const v = voices.find(x => x.name.includes("Evan") || x.name.includes("Alex") || x.name.includes("Daniel"));
                                  if (v) u.voice = v;
                                  u.rate = 1.1;
                      ...
                      >
                        <input
                          ...
                          placeholder="Yo, Signal — what should I do with this?"
                  ...
                  {agentRunning && (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground font-mono uppercase tracking-wider">
                      <span className="animate-pulse">●</span> Yo, Signal is working on it…
                    </div>
                  )}
                </div>
              )}
